...
|---|---|---|---|
| v1 | **0.04442** | LightGBM regressor, CV R²=0.2420 | Baseline, label-encoded genres. **ベストスコア** |
| v2 | 0.02135 | LightGBM regressor, CV R²=0.9763 | Target encoding (leakage あり) |
| v3 | -0.00893 | LightGBM regressor, CV R²=0.4307 | fold-internal target encoding (汎化失敗) |
| v4 | (未提出) | LightGBM+CatBoost ensemble, CV R²=0.4538 | label encoding + freq encoding + date features + interactions, target encoding なし。提出制限で未提出、次回提出予定 |

### Score Analysis

- v1 が最も良い。シンプルな label encoding + audio features が最も安定。
- Target encoding は train/val のカテゴリ分布が異なる場合に逆効果。
- v4: target encoding を完全に排除、freq encoding + label encoding + feature interactions + CatBoost ensemble で CV R² を 0.24 → 0.45 に大幅改善。LB score は未確認。
...
8. **Target encoding 不要で CV 改善**: v4 で target encoding を完全に排除し、freq encoding + label encoding（track_artist, track_album_id 含む）+ feature interactions + LightGBM+CatBoost ensemble で CV R² = 0.4538 を達成。v1 (0.24) から大幅改善。
9. **Kaggle 日次提出制限**: ap2026q1 は1日3回まで。UTC 基準でリセット。

### Next Steps

- v4 の Public LB score を確認（日次提出制限のため次回提出）
- feature importance を確認して不要な特徴量を削減
- XGBoost の追加でアンサンブルを強化
- ハイパーパラメータチューニング（Optuna）
- track_name からのテキスト特徴量（文字数、単語数等）
